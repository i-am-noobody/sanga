    ...
    <div className="space-y-6">
      <div className="rounded-[2rem] border border-red-300/20 bg-[#070707]/95 p-4 shadow-[0_24px_80px_-50px_rgba(218,41,28,0.35)] sm:p-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-red-300/70">Latest uploads</p>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          {uploads.length === 0 ? (
            ...
            uploads.slice(0, 6).map((image) => (
              <div
                key={image.asset_id}
                className="group overflow-hidden rounded-3xl border border-red-300/20 bg-[#0b0b0b]/90 transition hover:-translate-y-1 hover:border-red-300/40"
              >
                <img
      ...
      <div className="rounded-[2rem] border border-red-300/20 bg-[#070707]/95 p-4 shadow-[0_24px_80px_-50px_rgba(218,41,28,0.35)] sm:p-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-red-300/70">Recent menu items</p>
            <h2 className="mt-3 text-2xl font-semibold text-white">Dish catalogue</h2>
          </div>
          <span className="rounded-full border border-red-300/20 bg-white/5 px-3 py-1 text-xs uppercase tracking-[0.28em] text-slate-300">
        ...
        <div className="mt-6 grid gap-4">
          {menuItems.length === 0 ? (
            ...
            menuItems.slice(0, 6).map((item) => (
              <div key={item.id} className="rounded-3xl border border-red-300/20 bg-[#0b0b0b]/90 p-4 transition hover:-translate-y-1 hover:border-red-300/40">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <p className="font-semibold text-white">{item.name}</p>
                  <p className="rounded-full bg-red-400/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-red-200">
